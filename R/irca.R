...
#' @export irca.global.draw
irca.global.draw <- function(dataset, group) {
...
irca.global.cal <- function(dataset, group, threshold=0.6) {
  outliers <- outliers_maha_chisquare(dataset)
  if (length(outliers) != 0) {
    dataset <- dataset[-outliers,]
  }
...
#'
#' @export irca.local.draw
irca.local.draw <- function(dataset, bands_ann) {
  outliers <- outliers_maha_chisquare(dataset)
  if (length(outliers) != 0) {
    dataset <- dataset[-outliers,]
  }
...
irca.global <- function(object) {
  ...
  IRCA.interests <- lapply(unique(object@meta.data$group), function(x) {
    ...
    interests <- irca.global.draw(temp_data, x)
    dev.off()
    return(interests)
  })
...
irca.local <- function(object, bands_ann) {
  dataset <- get.nearest.dataset(object)
  ...
  lapply(unique(object@meta.data$group), function(x) {
    ...
    irca.local.draw(temp_data, bands_ann)
    grDevices::dev.off()
  })
}
